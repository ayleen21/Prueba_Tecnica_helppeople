...
namespace Catalogo.Api.Controllers
{
    ...
    public class ProductosController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateProducto(int id, [FromBody] ProductoUpdateDto dto)
        {
            ...
            // Validar SKU único solo si cambió (ignorando mayúsculas/minúsculas y espacios)
            var skuActual = (productoExistente.Sku ?? string.Empty).Trim().ToUpperInvariant();
            var skuNuevo = (dto.Sku ?? string.Empty).Trim().ToUpperInvariant();
            Console.WriteLine($"SKU actual: '{skuActual}' | SKU nuevo: '{skuNuevo}' | ID: {id}"); // LOG DEBUG
            if (skuActual != skuNuevo)
            {
                // Traer todos los SKUs de otros productos y comparar en memoria
                var otrosSkus = await _context.Productos
                    .Where(p => p.IdProducto != id)
                    .Select(p => p.Sku)
                    .ToListAsync();
                var existeRepetido = otrosSkus
            ...
            // Si el SKU recibido es nulo o vacío, conservar el SKU original
            if (string.IsNullOrWhiteSpace(dto.Sku))
            {
                dto.Sku = productoExistente.Sku ?? string.Empty;
            }
            ...
            var resultDto = new ProductoDto
            {
                ...
                Sku = productoExistente.Sku ?? string.Empty,
        ...
        public async Task<IActionResult> CargarMasivoCsv([FromServices] CsvProductoImporter importer, [FromForm] IFormFile archivo)
        {
            if (archivo == null || archivo.Length == 0)
                return BadRequest("No se recibió archivo.");
            using var stream = archivo.OpenReadStream();
            var (creados, errores, erroresDetalle) = await importer.ImportarCsvAsync(stream);
            return Ok(new { creados, errores, erroresDetalle });
        }
    }
}
